...
CV.dat <- function(dat = study0, datA = hist1, IDname, TrtInelname, Ename, 
  ...
  for (m in 1:M){
    ...
    # train.stacked$WT <- 1/predict(modT, newdata = dat.temp3, type = "survival") # same as the ratio
    dat.temp1$t1 <- 0
    ...
    res[m] <- IOC(predicted, test.stacked$Yik, test.stacked$deltaYik, test.stacked, modC, modT, "Harrell")
  }
...
IOC <- function(predicted, time, event, dat, modC, modT, method){ # method = "Harrell" or "Uno" (with IPTW)
  if (method == "Uno"){
    weights <- 1/predict(modC, newdata = dat, type = "survival")/predict(modT, newdata = dat, type = "survival")
  }
  num = 0
  denom = 0
  for (i in 1:nrow(dat)){
    if (method == "Harrell"){
      ...
      dat.temp <- dat
      dat.temp$X <- time[i]
      weights.temp <- 1/predict(modC, newdata = dat.temp, type = "survival")/predict(modT, newdata = dat.temp, type = "survival")
...
CV.dat(dat = study0, datA = hist1, IDname = "WL_ID_CODE", TrtInelname = "inactive", Ename = "dt_wl", 
       ...
       CSk = CSk, L = 90, M = 5, stratified = "add")
